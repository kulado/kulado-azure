...
## HEAD (Unreleased)
* Update kulado-terraform to fdafc00b3f
* Out-of-the-box Azure Cloud Shell support ([#74](https://github.com/kulado/kulado-azure/issues/74))
___
...
* Upgrade to kulado-terraform@98fabcf506

## 0.19.2 (2019-07-09)
* Fix detailed diffs with nested computed values.
* Communicate detailed information about the difference between a resource's desired and actual state during a Kulado update
...
* Update to kulado-terraform@3635bed3a5 which stops maps containing `.` being treated as nested maps.
...
## 0.18.6 (2019-06-07)
* Add TypeScript type guards for each resource class ([7ace3e9b5f](https://github.com/kulado/kulado-terraform/commit/7ace3e9b5f2dcd4692b029ba4b80360582d7949a))
...
## 0.18.4 (2019-05-22)
* Update the Kulado-Terraform bridge, fixing an issue where all resources were marked as needing recreation in some cases
...
* Fix a bug where omitting a location would result in the initial failure of `kulado preview`
* Update to v1.28.0 of the AzureRM Terraform provider
* Relax the maximum length of names on several resource types

## 0.18.2 (2019-05-06)
* Update the Kulado-Terraform bridge, adding non-null assertions to many properties in the TypeScript definitions for the Node.js SDK
...
* Add 'serverless' functionality in line with `@kulado/aws`. Specifically:
...
* The location for each resource attached to a provider instance is now set as part of the provider configuration. This means that the following code from v0.18.0:
  ...
  ```
  kulado config set azure:location EastUS
  ```
...
## 0.17.1 (2019-03-06)
* Depend on the latest `kulado` package in the `kulado_gcp` Python SDK

## 0.17.0 (2019-03-05)
* Update to the latest version of the `kulado` SDK
...
* Add support for the `deleteBeforeReplace` resource option and improved delete-before-replace behaviour introduced in Kulado v0.16.14
