...
import (
	...
	"github.com/kulado/kulado-terraform/pkg/tfbridge"
	"github.com/kulado/kulado/pkg/resource"
	"github.com/kulado/kulado/pkg/tokens"
	"github.com/kulado/kulado/pkg/util/contract"
...
// an MSI endpoint, but it also has Azure CLI installed. Therefore, to make Kulado CLI work
...
func Provider() tfbridge.ProviderInfo {
	...
	prov := tfbridge.ProviderInfo{
		P:           p,
		Name:        "azurerm",
		Description: "A Kulado package for creating and managing Microsoft Azure cloud resources.",
		Keywords:    []string{"kulado", "azure"},
		Homepage:    "https://kulado.io",
		License:     "Apache-2.0",
		Repository:  "https://github.com/kulado/kulado-azure",
		...
		Python: &tfbridge.PythonInfo{
			Requires: map[string]string{
				"kulado": ">=0.17.12,<0.18.0",
			},
		},
	}

	// TODO[kulado/kulado#280]: Until we can pass an Archive as an Asset, create a resource type
			...
			if tfs, has := schema.Schema[azureLocation]; has && (tfs.Optional || tfs.Required) {
				if _, hasfield := res.Fields[azureLocation]; !hasfield {
					...
					res.Fields[azureLocation] = &tfbridge.SchemaInfo{
						Name: azureLocation,
						Default: &tfbridge.DefaultInfo{
							From: func(res *tfbridge.KuladoResource) (interface{}, error) {
...
// IDEA: Consider moving this refactoring of AutoName to allow more flexible configuration back into kulado-terraform.
...
func FromName(options AutoNameOptions) func(res *tfbridge.KuladoResource) (interface{}, error) {
	return func(res *tfbridge.KuladoResource) (interface{}, error) {
