...
declare module "./queue" {
    interface Queue {
         ...
         */
        onEvent(
            name: string, args: ServiceBusCallback | QueueEventSubscriptionArgs, opts?: kulado.ComponentResourceOptions): QueueEventSubscription;
    }
}
...
export class QueueEventSubscription extends appservice.EventSubscription<ServiceBusContext, string, appservice.FunctionDefaultResponse> {
    ...
    constructor(
        ...
        args: QueueEventSubscriptionArgs, opts: kulado.ComponentResourceOptions = {}) {
...
export interface TopicEventSubscriptionArgs extends QueueEventSubscriptionArgs {
     ...
     */
    maxDeliveryCount?: kulado.Input<number>;
};
...
declare module "./topic" {
    interface Topic {
         ...
         */
        onEvent(
            name: string, args: ServiceBusCallback | TopicEventSubscriptionArgs, opts?: kulado.ComponentResourceOptions): TopicEventSubscription;
    }
}
...
export class TopicEventSubscription extends appservice.EventSubscription<ServiceBusContext, string, appservice.FunctionDefaultResponse> {
    ...
    constructor(
        ...
        args: TopicEventSubscriptionArgs, opts: kulado.ComponentResourceOptions = {}) {
        ...
        const subscription = args.subscription || new Subscription(name, {
            ...
            maxDeliveryCount: kulado.output(args.maxDeliveryCount).apply(c => c === undefined ? 10 : c),
        }, opts);
...
export class ServiceBusFunction extends appservice.Function<ServiceBusContext, string, appservice.FunctionDefaultResponse> {
    constructor(name: string, args: ServiceBusFunctionArgs) {
        ...
        const bindingConnectionKey = kulado.interpolate`ServiceBus${namespaceName}ConnectionKey`;
        ...
        const namespace = kulado.all([namespaceName, resourceGroupName])
                               .apply(([namespaceName, resourceGroupName]) =>
                                    getServiceBusNamespace({ name: namespaceName, resourceGroupName }));

        const appSettings = kulado.all([namespace.defaultPrimaryConnectionString, bindingConnectionKey]).apply(
...
export interface EventHubBindingDefinition extends appservice.BindingDefinition {
     ...
     */
    eventHubName: kulado.Input<string>;
     ...
     */
    consumerGroup?: kulado.Input<string>;
...
declare module "./eventHub" {
    interface EventHub {
         ...
         */
        onEvent(
            name: string, args: EventHubCallback | EventHubSubscriptionArgs, opts?: kulado.ComponentResourceOptions): EventHubSubscription;
    }
}
...
export class EventHubSubscription extends appservice.EventSubscription<EventHubContext, string, appservice.FunctionDefaultResponse> {
    ...
    constructor(
        ...
        args: EventHubSubscriptionArgs, opts: kulado.ComponentResourceOptions = {}) {
...
export class EventHubFunction extends appservice.Function<EventHubContext, string, appservice.FunctionDefaultResponse> {
    constructor(name: string, args: EventHubFunctionArgs) {
        ...
        // connection string is put into the app settings (under whatever key we want). Then, the
        // .connection property of the binding contains the *name* of that app setting key.
        const bindingConnectionKey = kulado.interpolate`EventHub${args.eventHub.namespaceName}ConnectionKey`;
        ...
        const namespace = kulado.all([args.eventHub.namespaceName, args.eventHub.resourceGroupName])
                               .apply(([namespaceName, resourceGroupName]) =>
...
export class EventGridCallbackSubscription<T> extends appservice.EventSubscription<EventGridContext<T>, EventGridEvent<T>, appservice.FunctionDefaultResponse> {
    ...
    constructor(name: string,
        ...
        const { resourceGroupName, location } =
            appservice.getResourceGroupNameAndLocation(args, kulado.output(scope.resourceGroupName));
        ...
        const keys = kulado.output(this.functionApp.getHostKeys());
        const key = keys.systemKeys["eventgrid_extension"];
        const url = kulado.interpolate`https://${this.functionApp.defaultHostname}/runtime/webhooks/eventgrid?functionName=${name}&code=${key}`;
...
export namespace events {
    ...
    export function onGridBlobCreated(name: string,
                                      ...
                                      opts?: kulado.ComponentResourceOptions): EventGridCallbackSubscription<eventgrid.StorageBlobCreatedEventData> {
    ...
    export function onGridBlobDeleted(name: string,
                                      ...
                                      opts?: kulado.ComponentResourceOptions): EventGridCallbackSubscription<eventgrid.StorageBlobDeletedEventData> {
        return new EventGridCallbackSubscription(
    ...
    export function onResourceGroupEvent(name: string,
                                         ...
                                         opts?: kulado.ComponentResourceOptions): EventGridCallbackSubscription<ResourceGroupEvent> {
        return new EventGridCallbackSubscription(
