 ...
 * 
 * const test = kulado.output(azure.network.getPublicIP({
 ...
 *     resourceGroupName: testResourceGroup.name,
 * });
 * const testPublicIP = kulado.all([testPublicIp.name, testVirtualMachine.resourceGroupName]).apply(([name, resourceGroupName]) => azure.network.getPublicIP({
 *     name: name,
 ...
 */
export function getPublicIP(args: GetPublicIPArgs, opts?: kulado.InvokeOptions): Promise<GetPublicIPResult> & GetPublicIPResult {
    const promise: Promise<GetPublicIPResult> = kulado.runtime.invoke("azure:network/getPublicIP:getPublicIP", {
        "name": args.name,
