...
interface FunctionAppArgsBase {
     ...
     */
    readonly codePathOptions?: kulado.runtime.CodePathOptions;
     ...
     * defaults to `8.11.1`.
     */
    readonly nodeVersion?: kulado.Input<string>;
...
function serializeFunctionCallback<C extends Context<R>, E, R extends Result>(
    args: CallbackArgs<C, E, R>): Promise<kulado.runtime.SerializedFunction> {
    ...
    return kulado.runtime.serializeFunction(func, { isFactoryFunction: !!args.callbackFactory });
}

function produceDeploymentArchive(args: MultiCallbackFunctionAppArgs): kulado.Output<kulado.asset.Archive> {
    return kulado.output(produceDeploymentArchiveAsync(args));
}

async function produceDeploymentArchiveAsync(args: MultiCallbackFunctionAppArgs): Promise<kulado.asset.AssetArchive> {
    const map: kulado.asset.AssetMap = {};
    map["host.json"] = new kulado.asset.StringAsset(JSON.stringify({
        version: "2.0",
    ...
    const pathSet = await kulado.runtime.computeCodePaths(args.codePathOptions);
    for (const [path, value] of pathSet.entries()) {
        map[path] = value;
    }

    for (const func of args.functions) {
        map[`${func.name}/function.json`] = kulado.output(func.bindings).apply(bs => new kulado.asset.StringAsset(JSON.stringify({
            disabled: false,
            bindings: bs,
        })));
        ...
        map[`${func.name}/index.js`] = new kulado.asset.StringAsset(`module.exports = require("./handler").handler`);
        map[`${func.name}/handler.js`] = new kulado.asset.StringAsset(body.text);
    }

    return new kulado.asset.AssetArchive(map);
}

function combineFunctionAppSettings(args: MultiCallbackFunctionAppArgs): kulado.Output<{ [key: string]: string }> {
...
export interface BindingSettings<T extends BindingDefinition> {
     ...
     */
    readonly binding: kulado.Input<T>;

    /**
     * A dictionary of application settings to be applied to the Function App.
     */
    readonly settings: kulado.Input<{ [key: string]: any; }>;
}
...
export abstract class Function<C extends Context<R>, E, R extends Result> {
     ...
     */
    public readonly bindings: kulado.Input<BindingDefinition[]>;
     ...
     */
    public readonly appSettings?: kulado.Input<{ [key: string]: string }>;

    constructor(name: string,
                trigger: kulado.Input<InputBindingDefinition>,
                args: CallbackFunctionArgs<C, E, R>,
                settings?: kulado.Input<{ [key: string]: string }>) {
...
export interface ArchiveFunctionAppArgs extends FunctionAppArgsBase {
     ...
     */
    archive: kulado.Input<kulado.asset.Archive>;
};
...
function createFunctionAppParts(name: string,
    ...
    const functionArgs: appservice.FunctionAppArgs = {
        ...
        appSettings: kulado.output(args.appSettings).apply(settings => {
            return {
                ...settings,
...
/**
  ...
  * https://github.com/kulado/docs/blob/master/reference/serializing-functions.md for additional
...
export class CallbackFunctionApp<C extends Context<R>, E, R extends Result> extends FunctionApp {
    ...
    constructor(name: string, bindingsOrFunc: kulado.Input<BindingDefinition[]> | Function<C, E, R>,
                args: CallbackFunctionAppArgs<C, E, R>, opts: kulado.CustomResourceOptions = {}) {
...
function getEndpoint(app: FunctionApp, rootPath: string) {
    return kulado.interpolate`https://${app.defaultHostname}/${rootPath}`;
}

/**
 * A base component for custom Function App components. It is required to provide a common parent
 * for all resources, so that they are logically grouped under the same root in the Kulado resource
 * tree.
 */
export abstract class PackagedFunctionApp extends kulado.ComponentResource {
    /**
 ...
 */
export abstract class EventSubscription<C extends Context<R>, E, R extends Result> extends kulado.ComponentResource {
    public readonly functionApp: CallbackFunctionApp<C, E, R>;

    constructor(type: string, name: string,
                bindingsOrFunc: kulado.Input<BindingDefinition[]> | Function<C, E, R>,
...
interface BaseSubscriptionArgs {
    ...
    location?: kulado.Input<string>;
}

/** @internal */
export function getResourceGroupNameAndLocation(
        args: BaseSubscriptionArgs, fallbackResourceGroupName: kulado.Output<string> | undefined) {
...
function combineAppSettings(settings: kulado.Input<{[key: string]: string}>[]): kulado.Output<{[key: string]: string}> {
    return kulado.all(settings).apply(items => items.reduce((a, b) => ({ ...a, ...b }), {}));
}
...
function combineBindingSettings(trigger: BindingSettings<BindingDefinition>,
    ...
    const bindings = kulado.all(all.map(bs => bs.binding));
...
declare module "./functionApp" {
    interface FunctionApp {
         ...
         */
        getHostKeys(): kulado.Output<FunctionHostKeys>;

        /**
         * Retrieve the keys associated with the given Function.
         */
        getFunctionKeys(functionName: kulado.Input<string>): kulado.Output<FunctionKeys>;
    }
}
...
FunctionApp.prototype.getFunctionKeys = function(this: FunctionApp, functionName) {
    return kulado.all([this.id, functionName]).apply(async ([id, functionName]) => {
        const credentials = await core.getServiceClientCredentials();
        const client = new AzureServiceClient(credentials);
