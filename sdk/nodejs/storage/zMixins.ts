 ...
 */
export function signedBlobReadUrl(blob: Blob | ZipBlob, account: Account): kulado.Output<string> {
    ...
    return kulado.all([account.name, account.primaryConnectionString, blob.storageContainerName, blob.name]).apply(
...
export interface BlobFunctionArgs extends appservice.CallbackFunctionArgs<BlobContext, Buffer, appservice.FunctionDefaultResponse> {
     ...
     * for more details.
     */
    filterPrefix?: kulado.Input<string>;
    filterSuffix?: kulado.Input<string>;
};
...
declare module "./container" {
    interface Container {
        ...
        onBlobEvent(name: string,
                    ...
                    opts?: kulado.ComponentResourceOptions): BlobEventSubscription;
...
export class BlobEventSubscription extends appservice.EventSubscription<BlobContext, Buffer, appservice.FunctionDefaultResponse> {
    constructor(
        ...
        args: BlobEventSubscriptionArgs, opts: kulado.ComponentResourceOptions = {}) {
...
export class BlobFunction extends appservice.Function<BlobContext, Buffer, appservice.FunctionDefaultResponse> {
    constructor(name: string, args: BlobFunctionArgs) {
        ...
        const path = kulado.interpolate`${args.container.name}/${prefix}{blobName}${suffix}`;
...
export interface BlobInputBindingArgs {
     ...
     */
    readonly blobName: kulado.Input<string>;
}

export class BlobInputBinding implements appservice.InputBindingSettings {
    public readonly binding: kulado.Input<BlobInputBindingDefinition>;
    public readonly settings: kulado.Input<{ [key: string]: any; }>;
    ...
    constructor(name: string, container: Container, args: BlobInputBindingArgs) {
        ...
        const path = kulado.interpolate`${container.name}/${args.blobName}`;
...
declare module "./queue" {
    interface Queue {
        ...
        onEvent(name: string,
                ...
                opts?: kulado.ComponentResourceOptions): QueueEventSubscription;
...
function resolveAccount(container: { storageAccountName: kulado.Output<string>, resourceGroupName: kulado.Output<string> }) {
    const connectionKey = kulado.interpolate`Storage${container.storageAccountName}ConnectionStringKey`;
    const account = kulado.all([container.resourceGroupName, container.storageAccountName])
                        .apply(([resourceGroupName, storageAccountName]) =>
                            storage.getAccount({ resourceGroupName, name: storageAccountName }));

    const settings = kulado.all([account.primaryConnectionString, connectionKey]).apply(
...
export class QueueOutputBinding implements appservice.OutputBindingSettings {
    public readonly binding: kulado.Input<QueueOutputBindingDefinition>;
    public readonly settings: kulado.Input<{ [key: string]: any; }>;
...
interface TableInputBindingDefinition extends TableBindingDefinition {
     ...
     */
    filter?: kulado.Input<string>;

    /**
     * The maximum number of entities to read.
     */
    take?: kulado.Input<number>;
}
...
export class TableInputBinding implements appservice.InputBindingSettings {
    public readonly binding: kulado.Input<TableInputBindingDefinition>;
    public readonly settings: kulado.Input<{ [key: string]: any; }>;
...
export class TableOutputBinding implements appservice.OutputBindingSettings {
    public readonly binding: kulado.Input<TableOutputBindingDefinition>;
    public readonly settings: kulado.Input<{ [key: string]: any; }>;
