...
class ScaleSet(kulado.CustomResource):
    automatic_os_upgrade: kulado.Output[bool]
    """
    ...
    """
    network_profiles: kulado.Output[list]
    """
    ...
    """
    overprovision: kulado.Output[bool]
    """
    ...
    """
    storage_profile_data_disks: kulado.Output[list]
    """
    A storage profile data disk block as documented below
    """
    storage_profile_image_reference: kulado.Output[dict]
    """
    A storage profile image reference block as documented below.
    """
    storage_profile_os_disk: kulado.Output[dict]
    """
    ...
    """
    upgrade_policy_mode: kulado.Output[str]
    """
    ...
    def __init__(__self__, resource_name, opts=None, automatic_os_upgrade=None, boot_diagnostics=None, eviction_policy=None, extensions=None, health_probe_id=None, identity=None, license_type=None, location=None, name=None, network_profiles=None, os_profile=None, os_profile_linux_config=None, os_profile_secrets=None, os_profile_windows_config=None, overprovision=None, plan=None, priority=None, resource_group_name=None, rolling_upgrade_policy=None, single_placement_group=None, sku=None, storage_profile_data_disks=None, storage_profile_image_reference=None, storage_profile_os_disk=None, tags=None, upgrade_policy_mode=None, zones=None, __name__=None, __opts__=None):
        ...
        :param kulado.Input[bool] automatic_os_upgrade: Automatic OS patches can be applied by Azure to your scaleset. This is particularly useful when `upgrade_policy_mode` is set to `Rolling`. Defaults to `false`.
        :param kulado.Input[dict] boot_diagnostics: A boot diagnostics profile block as referenced below.
        :param kulado.Input[str] eviction_policy: Specifies the eviction policy for Virtual Machines in this Scale Set. Possible values are `Deallocate` and `Delete`.
        :param kulado.Input[list] extensions: Can be specified multiple times to add extension profiles to the scale set. Each `extension` block supports the fields documented below.
        :param kulado.Input[str] health_probe_id: Specifies the identifier for the load balancer health probe. Required when using `Rolling` as your `upgrade_policy_mode`.
        :param kulado.Input[str] license_type: Specifies the Windows OS license type. If supplied, the only allowed values are `Windows_Client` and `Windows_Server`.
        ...
        :param kulado.Input[str] name: Specifies the name of the image from the marketplace.
        :param kulado.Input[list] network_profiles: A collection of network profile block as documented below.
        :param kulado.Input[dict] os_profile: A Virtual Machine OS Profile block as documented below.
        :param kulado.Input[dict] os_profile_linux_config: A Linux config block as documented below.
        :param kulado.Input[list] os_profile_secrets: A collection of Secret blocks as documented below.
        :param kulado.Input[dict] os_profile_windows_config: A Windows config block as documented below.
        :param kulado.Input[bool] overprovision: Specifies whether the virtual machine scale set should be overprovisioned.
        :param kulado.Input[dict] plan: A plan block as documented below.
        :param kulado.Input[str] priority: Specifies the priority for the Virtual Machines in the Scale Set. Defaults to `Regular`. Possible values are `Low` and `Regular`.
        :param kulado.Input[str] resource_group_name: The name of the resource group in which to create the virtual machine scale set. Changing this forces a new resource to be created.
        :param kulado.Input[dict] rolling_upgrade_policy: A `rolling_upgrade_policy` block as defined below. This is only applicable when the `upgrade_policy_mode` is `Rolling`.
        :param kulado.Input[bool] single_placement_group: Specifies whether the scale set is limited to a single placement group with a maximum size of 100 virtual machines. If set to false, managed disks must be used. Default is true. Changing this forces a new resource to be created. See [documentation](http://docs.microsoft.com/en-us/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-placement-groups) for more information.
        :param kulado.Input[dict] sku: Specifies the SKU of the image used to create the virtual machines.
        :param kulado.Input[list] storage_profile_data_disks: A storage profile data disk block as documented below
        :param kulado.Input[dict] storage_profile_image_reference: A storage profile image reference block as documented below.
        :param kulado.Input[dict] storage_profile_os_disk: A storage profile os disk block as documented below
        :param kulado.Input[dict] tags: A mapping of tags to assign to the resource.
        :param kulado.Input[str] upgrade_policy_mode: Specifies the mode of an upgrade to virtual machines in the scale set. Possible values, `Rolling`, `Manual`, or `Automatic`. When choosing `Rolling`, you will need to set a health probe.
        :param kulado.Input[list] zones: A collection of availability zones to spread the Virtual Machines over.
