...
#           would place them in `GOBIN`, but not `KULADO_ROOT`
#
#  - install: copies the bits we plan to ship into a layout in
#             `KULADO_ROOT` that looks like what a customer would get
#             when they download and install Kulado. For JavaScript
...
#              are often integration tests which will use `kulado` to
#              deploy example Kulado projects, creating cloud
...
#                     $(KULADO_ROOT)/node_modules/$(NODE_MODULE_NAME)
#
...
# Our install targets place items item into $KULADO_ROOT, if it's
# unset, default to /opt/kulado.
ifeq ($(KULADO_ROOT),)
	KULADO_ROOT:=/opt/kulado
endif

KULADO_BIN          := $(KULADO_ROOT)/bin
KULADO_NODE_MODULES := $(KULADO_ROOT)/node_modules
...
ifeq ($(PYTHON),)
	...
	$(error "missing python 2.7 (`python2` or `python`) from your $$PATH; \
		please see https://github.com/kulado/home/wiki/Package-Management-Prerequisites")
else
...
all:: build install lint test_all
	...
	@if [ -e 'Gopkg.toml' ]; then echo "dep ensure -v"; dep ensure -v; \
		elif [ -e 'go.mod' ]; then echo "GO111MODULE=on go mod vendor"; GO111MODULE=on go mod vendor; gomod-doccopy -provider terraform-provider-azurerm; fi
...
install::
	...
	cp -r bin/. "$(KULADO_NODE_MODULES)/$(NODE_MODULE_NAME)"
	cp yarn.lock "$(KULADO_NODE_MODULES)/$(NODE_MODULE_NAME)"
