...
# s3://rel.kulado.com/releases/plugins.
set -o nounset -o errexit -o pipefail

ROOT=$(dirname $0)/..
WORK_PATH=$(mktemp -d)
VERSION=$(jq -r '.version' < "${ROOT}/sdk/nodejs/bin/package.json")
PLUGIN_PACKAGE_NAME="kulado-resource-azure-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/kulado/kulado-azure/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/kulado-resource-azure${BIN_SUFFIX}" \
   "${ROOT}/cmd/kulado-resource-azure"

# Tar up the plugin
tar -czf ${PLUGIN_PACKAGE_PATH} -C ${WORK_PATH} .

# rel.kulado.com is in our production account, so assume that role first
CREDS_JSON=$(aws sts assume-role \
                 --role-arn "arn:aws:iam::058607598222:role/UploadKuladoReleases" \
                 --role-session-name "upload-plugin-kulado-resource-azure" \
                 --external-id "upload-kulado-release")
...
aws s3 cp --only-show-errors "${PLUGIN_PACKAGE_PATH}" "s3://rel.kulado.com/releases/plugins/${PLUGIN_PACKAGE_NAME}"
